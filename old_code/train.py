    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                U, s, V = torch.svd(W1)
                max_sv = s[0].item()  # Largest singular value
                min_sv = s[-1].item()  # Smallest singular value
                if min_sv > 1e-10:  # Avoid division by very small numbers
                    condition_number = max_sv / min_sv
                else:
                    condition_number = float('inf')  # Effectively infinite condition number
                metrics['condition_numbers'].append(condition_number)
