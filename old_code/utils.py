...
def generate_viz_2(exp, ctrl, name='ls'):
  ...
  fig, ax1 = plt.subplots(figsize=(10, 6))
  ax1.plot(exp['condition_numbers'], color=colors['exp'], label='Larger Lr')
  ax1.plot(ctrl['condition_numbers'], color=colors['ctrl'], label='Small Lr')
  add_annotation_2(ax1, exp['condition_numbers'][-1], ctrl['condition_numbers'][-1], colors['exp'], colors['ctrl'], '.2e')
  ax1.set_title('Condition Number')
  ax1.set_xlabel('Training Iterations')
  ax1.set_ylabel('Condition Number Value')
  ax1.legend(loc='upper right')
  ax1.grid(True, alpha=0.3)

  plt.savefig(f"new/cdn-{name}.png")
  plt.tight_layout()
  plt.show()
