...
from mnist import generate_data_mnist, train_model_mnist, generate_viz_mnist
...
def experiment7():
  for i in ['ls', 'ld', 'hd', 'hs']:
    ...
    for lr, wd in candidates:
      ...
      generate_viz_2(exp, ctrl, name=f'FINAL-{i}-{lr}-{wd}')

def experiment8():
  train_dataset, test_dataset = generate_data_mnist(NUM_SAMPLES)

  print("Training Catapult Model...")
  exp = train_model_mnist(train_dataset, test_dataset, 1.23, 100, weight_decay=0.00)

  print("Training Control Model...")
  ctrl = train_model_mnist(train_dataset, test_dataset, 0.01, 100, weight_decay=0.00)
